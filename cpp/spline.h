...
public:
    ...
    spline(const std::vector<tkreal_t>& X, const std::vector<tkreal_t>& Y,
           spline_type type = cspline,
           bool make_monotonic = false,
           bd_type left  = second_deriv, tkreal_t left_value  = 0.0,
           bd_type right = second_deriv, tkreal_t right_value = 0.0
          ):
        m_type(type),
    ...
    void set_boundary(bd_type left, tkreal_t left_value,
                      bd_type right, tkreal_t right_value);

    // set all data points (cubic_spline=false means linear interpolation)
    void set_points(const std::vector<tkreal_t>& x,
                    const std::vector<tkreal_t>& y,
                    spline_type type=cspline);
    ...
    tkreal_t operator() (tkreal_t x) const;
    tkreal_t deriv(int order, tkreal_t x) const;

    // returns the input data points
    std::vector<tkreal_t> get_x() const { return m_x; }
    std::vector<tkreal_t> get_y() const { return m_y; }
    tkreal_t get_x_min() const { assert(!m_x.empty()); return m_x.front(); }
    tkreal_t get_x_max() const { assert(!m_x.empty()); return m_x.back(); }
...
{
private:
    std::vector< std::vector<tkreal_t> > m_upper;  // upper band
    std::vector< std::vector<tkreal_t> > m_lower;  // lower band
public:
    ...
    // access operator
    tkreal_t & operator () (int i, int j);            // write
    tkreal_t   operator () (int i, int j) const;      // read
    // we can store an additional diagonal (in m_lower)
    tkreal_t& saved_diag(int i);
    tkreal_t  saved_diag(int i) const;
    void lu_decompose();
    std::vector<tkreal_t> r_solve(const std::vector<tkreal_t>& b) const;
    std::vector<tkreal_t> l_solve(const std::vector<tkreal_t>& b) const;
    std::vector<tkreal_t> lu_solve(const std::vector<tkreal_t>& b,
...
void spline::set_boundary(spline::bd_type left, tkreal_t left_value,
                          spline::bd_type right, tkreal_t right_value)
{
...
void spline::set_coeffs_from_b()
{
    ...
    for(size_t i=0; i<n-1; i++) {
        const tkreal_t h  = m_x[i+1]-m_x[i];
...
void spline::set_points(const std::vector<tkreal_t>& x,
                        const std::vector<tkreal_t>& y,
                        spline_type type)
{
    ...
    if(type==linear) {
        ...
        for(int i=1; i<n-1; i++) {
            const tkreal_t h  = m_x[i+1]-m_x[i];
            const tkreal_t hl = m_x[i]-m_x[i-1];
        ...
        if(m_right==first_deriv) {
            ...
            const tkreal_t h = m_x[n-1]-m_x[n-2];
...
bool spline::make_monotonic()
{
    ...
    for(int i=0; i<n-1; i++) {
        tkreal_t h = m_x[i+1]-m_x[i];
        tkreal_t avg = (m_y[i+1]-m_y[i])/h;
        if( avg==0.0 && (m_b[i]!=0.0 || m_b[i+1]!=0.0) ) {
            ...
            tkreal_t r = sqrt(m_b[i]*m_b[i]+m_b[i+1]*m_b[i+1])/std::fabs(avg);
            if(r>3.0) {
...
size_t spline::find_closest(tkreal_t x) const
{
    std::vector<tkreal_t>::const_iterator it;
    it=std::upper_bound(m_x.begin(),m_x.end(),x);       // *it > x
    size_t idx = std::max( int(it-m_x.begin())-1, 0);   // m_x[idx] <= x
    return idx;
}

tkreal_t spline::operator() (tkreal_t x) const
{
...
tkreal_t spline::deriv(int order, tkreal_t x) const
{
    assert(order>0);
    size_t n=m_x.size();
    size_t idx = find_closest(x);
...
tkreal_t & band_matrix::operator () (int i, int j)
{
...
tkreal_t band_matrix::operator () (int i, int j) const
{
    int k=j-i;       // what band is the entry
    assert( (i>=0) && (i<dim()) && (j>=0) && (j<dim()) );
    assert( (-num_lower()<=k) && (k<=num_upper()) );
...
tkreal_t band_matrix::saved_diag(int i) const
{
    assert( (i>=0) && (i<dim()) );
    return m_lower[0][i];
}
tkreal_t & band_matrix::saved_diag(int i)
{
    assert( (i>=0) && (i<dim()) );
    return m_lower[0][i];
}
...
// solves Ly=b
std::vector<tkreal_t> band_matrix::l_solve(const std::vector<tkreal_t>& b) const
{
    assert( this->dim()==(int)b.size() );
    std::vector<tkreal_t> x(this->dim());
    int j_start;
    tkreal_t sum;
    for(int i=0; i<this->dim(); i++) {
        sum=0;
...
// solves Rx=y
std::vector<tkreal_t> band_matrix::r_solve(const std::vector<tkreal_t>& b) const
{
    assert( this->dim()==(int)b.size() );
    std::vector<tkreal_t> x(this->dim());
    int j_stop;
...
std::vector<tkreal_t> band_matrix::lu_solve(const std::vector<tkreal_t>& b,
        bool is_lu_decomposed)
{
    assert( this->dim()==(int)b.size() );
    std::vector<tkreal_t>  x,y;
